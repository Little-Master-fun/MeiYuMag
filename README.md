...
用户申请美育场地时，直接上传 Word 申请文件。后台收到文件后交给 AI 初审，AI 主要负责从文件中提取申请房间、申请日期、申请时间段、借用组织等关键信息，并返回结构化检查结果。时间冲突等强规则由后端基于数据库中的预约记录进行判断，保证结果可复现、可审计。
...
| KeyBorrowRecord | 钥匙借用记录，用于保存 AI 提取出的借用组织、借用钥匙、借用开始时间和预计归还时间 |
