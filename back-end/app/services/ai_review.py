    ...
    def build_prompt(self, application_type: str) -> str:
        ...
        if application_type == "meiyu_venue":
            return (
                ...
                "场地别名对应关系：美育馆研讨室（西）即大学生研讨室1，美育馆研讨室（东）即大学生研讨室2。"
                "中英文括号均可，venue_name 请返回对应的正式名称大学生研讨室1或大学生研讨室2；"
                "同一场地的正式名称与别名不算多个场地，未写明东西或编号时不得猜测。"
                "美育馆允许延迟提交和补录，借用时间可以是今天或过去，不得仅因开始时间已过或活动已结束而拒绝。"
                "请按原件提取实际日期，不要将过去日期改为未来日期；结束时间须晚于开始时间。此补录规则不适用于悦园三楼。"
