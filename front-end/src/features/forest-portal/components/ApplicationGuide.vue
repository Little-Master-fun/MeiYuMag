...
const steps = [
  {
    icon: CalendarDays,
    title: '查看空闲',
    text: '先到场地日历查看空闲时间，再点击可申请日期或个人首页的“场地申请”前往信箱。将想申请的场地、日期和具体时段写在申请文件中，无需另外选择。',
  },
  {
    icon: FileText,
    title: '提交材料',
    text: '点击木牌上的材料名称，或到“示例与模板”下载文件。美育馆填写申请表，悦园三楼准备活动策划书，首次提交只需一份 DOCX，不用另附证明或说明材料。写清完整场地名称、日期和具体时段，点击信封或拖入文件，核对后点击“封缄并送出”。',
  },
  {
    icon: BadgeCheck,
    title: '预审核通过',
    text: '提交后回到个人首页查看申请进度。显示“待签章材料”时，按清单准备签字盖章文件；未通过时，查看原因，修改文件后从原申请重新提交。等待审核期间无需重复提交，预审核通过不代表最终获准使用。',
  },
  {
    icon: FileSignature,
    title: '提交签字盖章材料',
    text: '预审核通过后，按材料清单打印文件并完成签字盖章。在个人首页或申请档案中打开原申请，回到信箱上传清晰、完整的扫描件，可放入 Word 或 PDF 文件，须包含签字盖章页，不接受普通拍照照片。送出后继续查看进度，显示“已审核确认”才算申请成功。',
  },
]
const statuses = [
  { label: '待签章审核 / 待再次签章', tone: 'stone', text: '签章材料已收到，正在审核或办理后续签章。无需重复提交，请在个人首页查看后续进度。' },
  { label: '待人工初审', tone: 'stone', text: '材料已提交，请等待审核并在个人首页查看进度，无需重复上传。' },
  {
    label: '初审未通过',
    tone: 'rust',
    text: '打开原申请查看未通过原因，修改 DOCX 文件后点击“重新提交”，不必另建一份申请。',
  },
  {
    label: '待签章材料',
    tone: 'ochre',
    text: '按清单填写原表，打印并完成签字盖章，再上传扫描件；悦园三楼请同时准备清单要求的原文件与签章扫描件。此时还不能按申请使用场地。',
  },
  {
    label: '待管理员审核',
    tone: 'stone',
    text: '请等待最终审核结果。上传成功不代表申请成功，收到“已审核确认”后再按获批安排使用。',
  },
  {
    label: '待补交材料',
    tone: 'rust',
    text: '查看补交原因和材料清单，每种要求的材料准备一份文件，在原申请中补齐后再次送出。',
  },
  {
    label: '已审核确认',
    tone: 'sage',
    text: '申请已通过，请按获批的场地、日期和时段使用，并遵守场地管理要求。',
  },
  {
    label: '已完成或已取消',
    tone: 'stone',
    text: '在历史申请中查看记录。如不再使用，可在允许取消时打开申请档案取消；已被正式驳回的申请，如需再次申请，请重新发起。',
  },
]
async function load() {
  loading.value = true
  ...
  >
    ...
    >
      ...
      <nav class="guide-tabs" aria-label="指南章节">
        ...
        <button :aria-pressed="tab === 'rules'" @click="tab = 'rules'">02 进度与须知</button>
      ...
      <div ref="pageScroll" class="guide-scroll">
        <Transition name="guide-page" mode="out-in">
          ...
          <div v-else-if="tab === 'rules'" key="rules" class="guide-content">
            <p class="guide-intro">
              在个人首页查看申请状态，按下面的提示完成下一步。实际使用请以最终审核结果和场地管理规定为准。
            </p>
            ...
            <section class="guide-notes">
              <h2>寄出之前，再核对一下</h2>
              <p>
                请先完成账号认证并取得场地申请权限。提交前查看空闲时间，并核对文件里的场地、日期和时段；最终安排以审核确认结果为准。
              </p>
              <p>
                首次场地申请只需一份 DOCX；钥匙借用为一份 PDF。签章或按要求补交的材料支持
              ...
              <p>借用钥匙请点击个人首页的“钥匙申请”，参考图片示例填写钥匙名称、借用组织、借用人、联系方式、借还时间和事由，签名后提交一份 PDF，并在个人首页查看结果。钥匙借用和场地申请需分别提交。</p>
              <p><strong>签字盖章后请提交扫描件，须包含签字盖章页，可放入 Word 或 PDF 文件。</strong>不接受普通拍照照片。</p>
              <p>钥匙申请请提交清晰完整的 PDF 扫描件，不要直接上传普通拍照原图。示例图片仅供参考，不能直接作为申请提交。</p>
            </section>
          </div>
          <div v-else key="materials" class="guide-content">
            <p class="guide-intro">
              下载对应申请的模板，并填写自己的真实信息。“填写示例”仅供参考，不能原样提交；钥匙借用提供图片示例。纸质材料请填写并签名后扫描，不要直接提交普通拍照原图。
            </p>
